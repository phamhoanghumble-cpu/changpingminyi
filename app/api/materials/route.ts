...
export async function POST(request:Request){
 ...
 try{
  ...
  if(!isMaterialCategory(category))return Response.json({error:'请选择材料分类。'},{status:400});
  ...
  if(files.length>5||files.reduce((n,f)=>n+f.size,0)>20*1024*1024||files.some(f=>!extensions.has(f.name.split('.').pop()?.toLowerCase()||'')))return Response.json({error:'最多上传5个受支持的附件，总大小不超过20MB。'},{status:400});
  ...
  for(const f of files){const key='materials/'+id+'/'+crypto.randomUUID();await env.BUCKET!.put(key,f.stream(),{httpMetadata:{contentType:'application/octet-stream'}});saved.push(key);meta.push({key,name:f.name.slice(0,240),size:f.size,type:({'jpg':'image/jpeg','jpeg':'image/jpeg','png':'image/png','webp':'image/webp','mp4':'video/mp4','mov':'video/quicktime','mp3':'audio/mpeg','m4a':'audio/mp4','wav':'audio/wav','pdf':'application/pdf','txt':'text/plain'} as Record<string,string>)[f.name.split('.').pop()?.toLowerCase()||'']||'application/octet-stream'})}
  await db.prepare('INSERT INTO materials (id,title,content,location,event_date,created_at,files,file_count,status,consent,category) VALUES (?,?,?,?,?,?,?,?,?,?,?)').bind(id,title,content,location,eventDate,new Date().toISOString(),JSON.stringify(meta),files.length,'published',1,category).run();
